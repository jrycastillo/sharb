                ...
                <v-card class="card-width card-center">
                    <v-card-title primary-title>
                        <v-layout align-center justify-space-between row fill-height pa-3>
                            <v-flex class="headline" lg6>Approved Loading</v-flex>
                        </v-layout>
                    </v-card-title>
                                ...
                                <v-card flat v-for="detail in props.item.details" :key="detail.id">
                                    <v-card-text class="vcard-padd">
                                        <v-layout align-center justify-space-between row fill-height mx-3>
                                            ...
                                            <v-btn color="info" :href="'/approvedloading/' + detail.id ">View Detail
                                            </v-btn>
                                        </v-layout>
                                    </v-card-text>
                                </v-card>
...
@section('scripts')
    <script>
        new Vue({
            ...
            methods: {
                getData: function () {
                    axios.get("{{ url('/api/approvedloading') }}" + '?type=year&year=' + 2018)
                        .then(res => {
                            let data = [];
